...
%>
	...
	<div class="content"><!-- 상담 관리 -->
		<select name="prNo" onchange="movePage('Advice.jsp','prNo',this.value);"><!-- 과정 선택 -->
			<option></option>
			<%
				for(int i = 0; i < pList.size(); i++){
					if(pList.get(i).getPrStateName().equals("훈련중")){		//과정 상태가 훈련중만 출력
		...
		<table class="advice-table">					<!-- 해당 과정을 수강하는 학생 목록 -->
			...
			%>
						<tr class="advice-content-<%=style[i % 2]%>">
							<td rowspan=2><a class="advice-content-text-move" href="StudentInfo.jsp?stNo=<%=stNo %>"><%=stList.get(0).getStName() %></a></td>
							<td rowspan=2><%=stList.get(0).getStStateName() %></td>
							<td style="text-align:left;">
								<input type="hidden" id="stNo<%=stNo %>" name="stNo" value="<%=stList.get(0).getStNo()%>">
								<%
								...
								%>
										<input name="prStateName<%=stNo %>" type="radio" value="<%=psList.get(j).getPrStateName()%>"><%=psList.get(j).getPrStateName()%>
								<%
									}
								%>
							</td>
